...
export '../services/auth_service.dart'
    show authStateProvider, authServiceProvider, currentUserProvider;
...
class ProjectsNotifier extends Notifier<List<Project>> {
  ...
  FirestoreRepository get _repo => ref.read(paraRepositoryProvider);
  ...
  Future<void> _init() async {
    try {
      ...
      debugPrint('[ProjectsNotifier] _init error: $e');
    }
  }
...
/// Areas Provider
/// ─────────────────────────────────────────
final areasProvider = NotifierProvider<AreasNotifier, List<Area>>(
  AreasNotifier.new,
);
...
class AreasNotifier extends Notifier<List<Area>> {
  ...
  Future<void> _init() async {
    try {
      ...
      debugPrint('[AreasNotifier] _init error: $e');
    }
  }
...
class ResourcesNotifier extends Notifier<List<Resource>> {
  ...
  Future<void> _init() async {
    try {
      ...
      debugPrint('[ResourcesNotifier] _init error: $e');
    }
  }
...
/// Inbox Provider
/// ─────────────────────────────────────────
final inboxProvider = NotifierProvider<InboxNotifier, List<InboxItem>>(
  InboxNotifier.new,
);
...
class InboxNotifier extends Notifier<List<InboxItem>> {
  ...
  Future<void> _init() async {
    try {
      ...
      debugPrint('[InboxNotifier] _init error: $e');
    }
  }
...
final _sampleAreas = [
  Area(
    id: _uuid.v4(),
    title: '커리어 개발',
    description: '기술 역량 향상 및 경력 관리',
    icon: Icons.work_outline,
    standard: '매주 학습 5시간 이상',
